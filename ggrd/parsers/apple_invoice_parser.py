...
class HtmlCleaner:
    def __init__(self, html_content: str):
        ...
        item_parts = []
        if item_cell:
            title = item_cell.find("span", class_="title")  # type: ignore
            artist = item_cell.find("span", class_="artist")  # type: ignore
            item_type = item_cell.find("span", class_="type")  # type: ignore
            device = item_cell.find("span", class_="device")  # type: ignore
            ...
            if found_br or sibling_counter <= 2:  # Check immediately or after <br>
                ...
                # 2. Handle nested tag case (e.g., Invoice Date in a span)
                    ...
                    descendant_text = sibling.find(
                        string=lambda t: self.get_cleaned_text(t)  # type: ignore
                    )
                    if descendant_text:
